...
class SampleInteractor(
    ...
    fun task1(): Flow<String> {
        return sampleRepository.produceNumbers()
            .map { it * 5 } // 1) умножаем на 5
            .filter { it > 20 } // 2) убираем числа <= 20
            .filter { it % 2 != 0 } // 3) убираем четные числа
            .map { "$it won" } // 4) добавляем постфикс "won"
            .take(3) // 5) берем 3 первых числа
    }
    ...
    fun task2(): Flow<String> {
        return sampleRepository.produceNumbers()
            .flatMapConcat { number ->
                when {
                    number % 15 == 0 -> flowOf(number.toString(), "FizzBuzz")
                    number % 5 == 0 -> flowOf(number.toString(), "Buzz")
                    number % 3 == 0 -> flowOf(number.toString(), "Fizz")
                    else -> flowOf(number.toString())
                }
            }
    }
        ...
        return sampleRepository.produceNumbers()
            .catch { e ->
                if (e is IllegalArgumentException) {
                    ...
                    throw e // пробрасываем другие исключения дальше
                }
            }
            .onCompletion {
